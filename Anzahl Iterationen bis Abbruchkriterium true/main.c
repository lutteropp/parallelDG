...
void jacobiSerial(const float* startVector, float h, const float* functionTable, float* jacobiResult)
{
    ...
    for (k = 0; k < MAX_ITERATIONS; ++k)
    {
        ...
        if (diff / (size * size) < TOL) {
        	//printf("Jacobi hat %d Iterationen gebraucht.\n", k+1);
        	count_jacobi += k+1;
        	break;
        }
    }
...
void gaussSeidel(const float * startVector, float h, const float* functionTable, float* gaussSeidelResult)
{
    ...
    for (k = 0; k < MAX_ITERATIONS; ++k)
    {
        ...
        if (diff / (size * size) < TOL) {
        	count_gs += k+1;
        	//printf("GS hat %d Iterationen gebraucht.\n", k+1);
        	break;
        }
    }
...
/*
    ...
    printf("count_jacobi = %.3f\n", (float) count_jacobi/ (float) repeats);
    printf("count_gs = %.3f\n", (float) count_gs/ (float) repeats);
    return 0;
}
