...
$$u^{(m)} = -D^{-1}(L+R)  u^{(m-1)} + D^{-1}b$$
$$\Leftrightarrow u^{(m)} = D^{-1}  \left(b -(L+R)  u^{(m-1)}\right)$$

Oder elementweise:
$$u_i^{(m)} = \frac{1}{a_{i,i}} * (b_i - \sum_{j \neq i}{a_{ij} u_j^{(m-1)}}) \quad \forall i = 1, \ldots, size$$
...
$$Au = b$$
$$\Leftrightarrow (D+L+R)u = b$$
$$\Leftrightarrow (L+D)u = b - Ru$$
$$\Leftrightarrow u = (L+D)^{-1} (b - Ru)$$

TODO: ...

Dies führt zu folgendem iterativen Verfahren, wobei $u^{(k)}$ den Vektor $u$ in Iteration $k$ meint:

$$u^{(m)} = -D^{-1} \left(b - Lu^{(m)} - Ru^{(m-1)}\right)$$

Oder elementweise:
$$u_i^{(m)} = \frac{1}{a_{i,i}} * \left(b_i - \sum_{j=1}^{i-1}{a_{i,j} u_j^{(m)}} - \sum_{j=i+1}^{size}{a_{i,j}*u_i^{(m-1)}}\right) \quad \forall i = 1, \ldots, size$$

Der Startvektor $u^{(0)}$ kann hierbei beliebig gewählt werden und es gilt, dass $(L+D)$ invertierbar sein muss. Dies ist jedoch bei unserer Aufgabenstellung der Fall, da TODO
