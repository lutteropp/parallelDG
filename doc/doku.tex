...
Oder elementweise:
$$u_i^{(k)} = \frac{1}{a_{i,i}} * \left(b_i - \sum_{j=1}^{i-1}{a_{i,j} u_j^{(k)}} - \sum_{j=i+1}^{n^2}{a_{i,j}*u_j^{(k-1)}}\right) \quad \forall i = 1, \ldots, n^2$$
...
Beide Verfahren sind uneingeschränkt stabil (\url{http://www.aia.rwth-aachen.de/vlueb/vl/numerische_stroemungsmechanik_ii/material/msl.pdf}, Seite 52f).

Da unsere Matrix $A$ strikt diagonaldominant ist, konvergieren sowohl das Jacobi- als auch das Gauß-Seidel-Verfahren in unserer Anwendung (\url{http://www.mathematik.uni-muenchen.de/~lerdos/SS08/Num/9.pdf}, Seite 4 und 8). Hierbei konvergiert das Gauß-Seidel-Verfahren deutlich schneller als das Jacobi-Verfahren, da das Jacobi-Verfahren im Gegensatz zum Gauß-Seidel-Verfahren mit veralteten Näherungen weiterrechnet.
...
\begin{center}
\includegraphics[scale=0.7]{bilder/fehler_jacobi_1_4.png}
\end{center}

\begin{center}
\includegraphics[scale=0.7]{bilder/fehler_gs_1_4.png}
\end{center}

\begin{center}
\includegraphics[scale=0.7]{bilder/fehler_jacobi_1_8.png}
\end{center}

\begin{center}
\includegraphics[scale=0.7]{bilder/fehler_gs_1_8.png}
\end{center}

\begin{center}
\includegraphics[scale=0.7]{bilder/fehler_jacobi_1_16.png}
\end{center}

\begin{center}
\includegraphics[scale=0.7]{bilder/fehler_gs_1_16.png}
\end{center}

\begin{center}
\includegraphics[scale=0.7]{bilder/fehler_jacobi_1_32.png}
\end{center}
...
Unsere Messungen zeigen, dass bei hinreichend hoher Problemgröße (also kleinem $h$) unsere Implementierung gut mit der Anzahl verfügbarer Threads skaliert. Ist die Problemgröße jedoch zu klein gewählt, führen mehr Threads nur noch zu einer geringen zusätzlichen Beschleunigung.

Um die Skalierbarkeit hinsichtlich der Problemgröße zu bewerten, haben wir unsere Gauß-Seidel Rot-Schwarz SSE Implementierung mit verschiedenen Größen des Lösungsvektors $u$ getestet. In Abbildung~\ref{fig:skalierbarkeit} ist zu sehen, dass der erforderliche Zeitaufwand nahezu linear in der Größe des Lösungsvektors $u$ ist. Zur Bestimmung der Messwerte haben wir verwendet, dass $u$ insgesamt $(size-2)^2$ Elemente enthält.

\begin{figure}[h!]
\centering
\includegraphics[scale=0.7]{bilder/skalierbarkeit_u.png}
\caption{Laufzeit von Gauß-Seidel Rot-Schwarz SSE mit 8 Threads für verschiedene Größen von $u$ auf \texttt{i82sn02.itec.kit.edu}}
...
In den folgenden Messungen wurde der Speedup immer relativ zu unseren sequentiellen Implementierungen gemessen. Die Messwerte können leicht von den bisher angegebenen abweichen, da sie teils mit anderen Werten für \texttt{TOL} ausgeführt wurden (in den alten Performance-Messungen hatten wir \texttt{TOL} = $0,00000001$ benutzt) und das Abbruchkriterium dadurch erst später erreicht wurde. %Außerdem hatten wir bei den alten Messungen noch nicht beachtet, dass man mit \texttt{srand(0)} den Zufallsgenerator zurücksetzen sollte, um immer den selben Zufalls-Startvektor für die Messungen zu erhalten.
\\
