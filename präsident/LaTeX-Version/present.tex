...
\begin{itemize}
\item Jacobi-Verfahren: $$u_i^{(k)} = \frac{1}{a_{i,i}} * \left(b_i - \sum_{j \neq i}{a_{ij} u_j^{(k-1)}}\right) \quad \forall i = 1, \ldots, n^2$$
\item Gauß-Seidel-Verfahren: $$u_i^{(k)} = \frac{1}{a_{i,i}} * \left(b_i - \sum_{j=1}^{i-1}{a_{i,j} u_j^{(k)}} - \sum_{j=i+1}^{n^2}{a_{i,j}*u_j^{(k-1)}}\right) \quad \forall i = 1, \ldots, n^2$$
\end{itemize}
